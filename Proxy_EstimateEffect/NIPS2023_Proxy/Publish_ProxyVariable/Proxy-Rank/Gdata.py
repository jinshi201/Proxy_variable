...
def Gdata1(Num=3000):
    ...
    W1 = SelectPdf(Num)*Toa()+U*ToBij()



    Y = SelectPdf(Num)*Toa()+U*ToBij()+X*1.2+X1*0.8+W*ToBij()+W1*ToBij()


    data2 = pd.DataFrame(np.array([X,Y,Z,W,Z1,W1,X1]).T,columns=['X','Y','Z','W','Z1','W1','X1'])
    #data2 = pd.DataFrame(np.array([X,Y,Z,W,Z1]).T,columns=['X','Y','Z','W','Z1'])
